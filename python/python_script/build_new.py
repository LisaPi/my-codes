...
def execute_ovs_make(server_ip=None, branch_name=None, 
    ...
    commands = [
                ...
                'tar -czvf /tftpboot/%s/daily/%s/ovs_bin-%s-%s-%s.tar.gz ./vswitchd/ovs-vswitchd.dbg ./ovsdb/ovsdb-server.dbg ./utilities/ovs-appctl.dbg ./utilities/ovs-vsctl.dbg ./utilities/ovs-ofctl.dbg' % (user_name, model_name, reversion_id, model_name, current_version)
                ]
...
if len(sys.argv) < 6:
    print "********You are missing some parameters********"
    print Usage
    sys.exit(1)
elif len(sys.argv) == 6:
