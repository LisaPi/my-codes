    ...
    for model in model_type_name:
        if model in ['as5712_54x', 'niagara2632xl']:
            ...
            pica_config_name[branch][model] = './configure --prefix=%s/%s/%s/%s/rootfs-debian/%s \
                                        ...
                                        --with-chip_sdk_deprecated_version=no \
                                        --with-chip_sdk_root_path=%s/%s/sdk/%s \
                                        ' % (sDirs, hTmp, oDir, box_name_map[model], pica_xorp_map[branch], hType, sNoDug, sOpt, '3296' ,sDirs, box_map[model], sDirs, hTmp, sLib)                                         
        else:
            sDirs = '/home/%s/%s/release/pica8/branches/%s' % (user_name, re.search("([0-9]{4})", model).group(), branch)
            hTmp = rel_dir_name[branch]
            oDir = os_dir_name[branch]
...
def execute_svn_update(server_ip=None, svn_type=None, branch_name=None, model_name=None):
    ...
    # Sdk update
    execute_send_expect(child=child, commands=['rm -rf %s/sdk/%s' % (rel_name_map[branch_name],
                                                                    sdk_loc_name[model_name])])
    execute_send_expect(child=child, commands=['svn update %s %s/sdk/%s' % (sdk_id_map[branch_name],
...
def execute_xorp_make(server_ip=None, branch_name=None, 
...
def execute_image_copy(server_ip=None, branch_name=None, model_name=None, reversion_id=None, version_number=None):
    ...
    exeecute_send_expect(child=child, commands=commands)
...
def execute_main(server_ip=None, svn_type=None, branch_name=None, 
    ...
    print "\n********PICA Make**********\n"   
    execute_xorp_make(server_ip=server_ip, 
                     branch_name=branch_name, 
