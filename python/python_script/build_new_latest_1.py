...
supporting models:3290 3295 3297 3922 3924 3930 3780 5401 es4654bf as6701_32x as5712_54x niagara2632xl arctica4804i niagara2948_6xl HP5712 as4610 dcs7032q28 as6712_32x HP6712 as7512_32x
...
SMTPSVR = '10.10.50.11'
branch_id = ['1.1','2.6','verizon','2.6B','netconfig','2.6.4','2.6.5']
model_type = ['3290', '3295', '3297', '3920', '3922', '3924', '3930', '3780', '5401', '5101', 'es4654bf', 'as6701_32x', 'as5712_54x', 'niagara2632xl', 'arctica4804i','niagara2948_6xl', 'HP5712', 'as4610', 'dcs7032q28','as6712_32x','HP6712','as7512_32x']
...
for branch in branch_id:
    ...
    for model in model_type:
        if model in ['as5712_54x', 'niagara2632xl', 'niagara2948_6xl','HP5712','as6712_32x','HP6712']:
            ...
            pica_config_name[branch][model] = './configure --prefix=%s/%s/%s/%s/rootfs-debian/%s \
                                        ...
                                        --with-rootfs_debian=yes \
                                        ovs_cv_use_linker_sections=yes \
                                        --with-chip_sdk_lib_path=%s/pica/exlib/xdk_3.1.0 \
                                        --with-chip_sdk_deprecated_version=no \
                                        --with-chip_sdk_root_path=%s/%s/sdk/%s \
                                        ' % (sDirs, hTmp, oDir, box_name_map[model], pica_xorp_map[branch], hType, sNoDug, sOpt, '3296' ,sDirs, sDirs, hTmp, sLib)
        elif model in ['as7512_32x']:
            sDirs = '/home/%s/%s/release/pica8/branches/%s' % (user_name, re.search("([0-9]{4})", model).group(), branch)
            hTmp = rel_name_map[branch]
            oDir = os_name_map[branch]
            hType = cross_compile_map[model]
            ...
            pica_config_name[branch][model] = './configure --prefix=%s/%s/%s/%s/rootfs-debian/%s \
                                        ...
                                        --with-rootfs_debian=yes \
                                        ovs_cv_use_linker_sections=yes \
                                        --with-chip_sdk_lib_path=%s/pica/exlib/xdk-rel-3-1\
                                        --with-chip_sdk_deprecated_version=no \
                                        --with-chip_sdk_root_path=%s/%s/sdk/%s \
                                        '  % (sDirs, hTmp, oDir, box_name_map[model], pica_xorp_map[branch], hType, sNoDug, sOpt, model_dir_name[model], sDirs,sDirs, hTmp, sLib)         
        else:
            sDirs = '/home/%s/%s/release/pica8/branches/%s' % (user_name, re.search("([0-9]{4})", model).group(), branch)
            hTmp = rel_name_map[branch]
            oDir = os_name_map[branch]
...
for branch in branch_id:
    ...
    for model in model_type:
        if model in ['as5712_54x', 'es4654bf', 'as6701_32x', 'niagara2632xl', 'niagara2948_6xl','HP5712','as4610','dcs7032q28','as6712_32x','HP6712','msh8910','as7512_32x']:
            hType = cross_compile_map[model]
...
image_dir_name = {'3290': '/tftpboot/%s/daily/3290' % user_name,
               ...
               '3296': '/tftpboot/%s/daily/3297' % user_name,
               ...
               'as4610':'/tftpboot/%s/daily/as4610' % user_name,
               'dcs7032q28':'/tftpboot/%s/daily/dcs7032q28' % user_name,
               'as6712_32x':'/tftpboot/%s/daily/as6712_32x',%user_name,
               'HP6712':'/tftpboot/%s/daily/as6712_32x',%user_name,
               'as7512_32x':'/tftpboot/%s/daily/as7512_32x',%user_name,
               'msh8910':'/tftpboot/%s/daily/msh8910',%user_name,
...
###### Svn Cleanup/Update
def execute_svn_update(server_ip=None, svn_type=None, branch_name=None, model_name=None):
    ...
    if svn_type == 0:
        ...
        commands = [ 
                'ls | grep -v os-dev | grep -v sdk | xargs chown -R %s' % user_name,
                'ls | grep -v os-dev | grep -v sdk | xargs chgrp -R %s' % user_name,    
                'chown -R %s %s/%s/%s' % (user_name, rel_name_map[branch_name], os_name_map[branch_name], box_name_map[model_name]),
                'chgrp -R %s %s/%s/%s' % (user_name, rel_name_map[branch_name], os_name_map[branch_name], box_name_map[model_name])]
...
def execute_os_make(server_ip=None, branch_name=None, 
    ...
    if model_name not in ['as5712_54x', 'niagara2632xl','niagara2948_6xl','HP5712','dcs7032q28','as6712_32x','HP6712','as7512_32x']:
        commands = [
                'export ARCH=powerpc',
                ]
        execute_send_expect(child=child, commands=commands) 
    ...
    if model_name in ['as5712_54x', 'niagara2632xl', 'niagara2948_6xl','as4610','dcs7032q28','as6712_32x','as7512_32x']:
    ...
    elif model_name in ['HP5712','HP6712']:
        commands = [
                'sudo make fast PICA8=1 %s=1 SDK=%s/%s/sdk/%s BRANCH=%s%s' % (onie_name_map[model_name], sDirs, hTmp,
                                                                              sLib,branch_name,sRevison)
                ]           
    else:
        lBox = string.upper(box_name_map[model_name])
...
def execute_sdk_make(server_ip=None, branch_name=None, 
    ...
    if model_name in ['as5712_54x', 'niagara2632xl', 'niagara2948_6xl','as4610','dcs7032q28','as6712_32x','HP6712','as7512_32x']:
        lBox = string.upper(box_name_map[model_name])
        commands = [
        ...
        commands = [
                'sudo make BCM_SDK_ALL  PICA8=1 %s=1 SDK=%s/%s/sdk/%s BRANCH=%s%s' % (onie_name_map[model_name], sDirs, hTmp,
                                                                              sLib,branch_name,sRevison)
                ]        
    else:
        lBox = string.upper(box_name_map[model_name])
...
def execute_image_copy(server_ip=None, branch_name=None, model_name=None, reversion_id=None, version_number=None):
    ...
    # Do the copy
    if model_name in ['as5712_54x', 'niagara2632xl','niagara2948_6xl','dcs7032q28','as6712_32x','HP6712','as7512_32x']:
        onie_platform = 'x86'
        copyCmd = 'scp'
...
def execute_main(server_ip=None, svn_type=None, branch_name=None, 
    ...
    if model_name in ['as6712_32x'] and server_ip not in ["10.10.50.22"]:
        print 'platform of as6712 image can only be built on 10.10.50.22!' 
        sys.exit()
        
    if model_name in ['HP6712'] and server_ip not in ["10.10.50.22"]:
        print 'platform of HP6712 image can only be built on 10.10.50.22!'
        sys.exit()

    if model_name in ['as7512_32x'] and sever_ip not in ['10.10.50.24']:
       print 'platform of as7512_32x image can only be built on 10.10.50.24!' 
       sys.exit()
...
if len(sys.argv) < 7:
    ...
    branch_name,server_ip,model_name,svn_type,is_license,is_sdk,brand_name,company_name,platform_name,host_name,sbrand_name= [w for w in sys.argv[1:]]
    execute_main(server_ip=server_ip, svn_type=svn_type, branch_name=branch_name, 
    ...
    branch_name,server_ip,model_name,svn_type,is_license,is_sdk,brand_name,company_name,platform_name,host_name,sbrand_name,depend_version= [w for w in sys.argv[1:]]
    execute_main(server_ip=server_ip, svn_type=svn_type, branch_name=branch_name, 
