...
supporting models, 3290 3295 3297 3922 3924 3930 3780 5401 es4654bf as6701_32x as5712_54x
...
model_type_name = ['3290', '3295', '3297', '3920', '3922', '3924', '3930', '3780', '5401', '5101', 'es4654bf', 'as6701_32x', 'as5712_54x', 'niagara2632xl']
...
def excute_xorp_make(server_ip=None, branch_name=None, 
    ...
    sDirs = '/home/%s/%s/release/pica8/branches/%s' % (user_name, model_name, branch_name)
...
def excute_ovs_make(server_ip=None, branch_name=None, 
    ...
    if is_ovs_xovs == 'ovs':
        commands = [
                ...
                'make -j8',
                'make install',
                'cp vswitchd/ovs-vswitchd /tftpboot/%s/daily/%s/ovs-vswitchd.%s.%s-%s' % (user_name, model_name, reversion_id, current_version, model_name),
                'make install-strip'
                ]
...
def excute_os_make(server_ip=None, branch_name=None, 
    ...
    if model_name in ['as5712_54x', 'niagara2632xl']:
        ...
        lBox = string.upper(box_name_map[model_name])
        commands = [
                'sudo make fast %s=1 SDK=%s/%s/sdk/%s BRANCH=%s%s' % (lBox, sDirs, hTmp,
                                                                      sLib,branch_name,sRevison)  
                    ]      
...
def excute_image_copy(server_ip=None, branch_name=None, model_name=None, reversion_id=None, version_number=None):
    ...
    if branch_name in ['2.1-mezocliq']:
        ...
        commands = ['%s build_image/pica.tar.gz %s/%s/pica-%s-%s-%s.tar.gz' % (copyCmd, 
                                                                               dirIp,
                                                                               image_dir_name[model_name],
                                                                               reversion_id,
