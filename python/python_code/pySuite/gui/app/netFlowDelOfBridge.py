...
@author: beyondzhou
@name: netFlowDelOfBridge.py
'''

def netFlowDelOfBridge():
    
    from login import loginCli, loginPha
    from cliCommLib import cliBridgeCmd, cliBridgeReset, cliPortsIdList
    from guiCommLib import guiAddBridge, guiAddPortIntoBridge, guiAddNetFlowIntoBridge, guiDelNetFlowOfBridge
    import re
    import myglobals
    import time
    ...
    print ':::Step 4: del netFlow of bridges through gui'  
    bridge = "br0"
    
    browser = loginPha()
    guiDelNetFlowOfBridge(browser, 
                           bridge=bridge)  
