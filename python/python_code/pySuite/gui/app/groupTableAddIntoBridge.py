...
@author: beyondzhou
@name: groupTableAddIntoBridge.py
'''

def groupTableAddIntoBridge():
    
    from login import loginCli, loginPha
    from cliCommLib import cliBridgeCmd, cliBridgeReset, cliPortsIdList
    from guiCommLib import guiAddBridge, guiAddPortIntoBridge, guiAddGroupTableIntoBridge
    import re
    import myglobals
    import time
    ...
    print ':::Step 3: get group table information through cli'
    time.sleep(1)
    ...
    if re.search('group_id=%s,type=%s,bucket=actions=%s' % (groupid, grouptype, actions), subject):
        print 'Group table check pass for brige:%s' % bridge
    else:
        print 'Group table check fail for brige:%s' % bridge
