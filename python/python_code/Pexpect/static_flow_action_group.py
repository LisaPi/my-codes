...
#Add a static flow with flow action group
child.sendline('./ctrlr-vsctl -- set Bridge sw0_br0 groups:0=@gp -- --id=@gp create Group name=g0 id=0')
child.expect('ctrlr-vsctl')
print child.before
child.sendline('./ctrlr-vsctl -- set Group g0 type=all buckets=@bt -- --id=@bt create Bucket name=bt0')
child.expect('ctrlr-vsctl')
print child.before
child.sendline('./ctrlr-vsctl -- set Bucket bt0 actions:output=2')
child.expect('ctrlr-vsctl')
print child.before

child.sendline('./ctrlr-vsctl -- add Flow_Table table0 static_flows @sf2 -- --id=@sf2 create Static_Flow name=sf4 match:{in_port=1,eth_type=0x0800,eth_dst=22:22:22:22:22:22} actions:group=0')
child.expect('ctrlr-vsctl')
print child.before
child.sendline('./ctrlr-vsctl list-group sw0_br0')
child.expect('ctrlr-vsctl')
