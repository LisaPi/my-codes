...
# Create another multi flows
number = 1000
macs = geneMac('22:00:00:00:00:00', "%d" % number)
sf = 2

for mac in macs:
    sendExpect(child, './ctrlr-vsctl -- add Dynamic_Flow df0 flows @flow -- --id=@flow create Static_Flow name=sf%s match:eth_dst=%s actions:output=all' % (sf, mac), prompt)
    sf = sf + 1

# List dynamic and static flow
...
# Do the flow check
result = checkCmd(switch, 'ovs-ofctl dump-flows br0 | grep actions | wc -l', '%s' % (number+1) )
if result:
    print 'Dynamic multi flow check Pass!'
else:
    print 'Dynamic multi flow check Fail!'
