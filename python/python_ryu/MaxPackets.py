...
class MaxPackets(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_2.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(MaxPackets, self).__init__(*args, **kwargs)
        self.mac_to_port = {} 
    ...
    def installDefaultFlow(self,datapath):
        while self.vlanid <= self.VlanID:
            ...
            self.flow_v12(datapath, priority=1000, match=match,actions=actions)
            self.vlanid = self.vlanid + 1
        else:
            self.vlanid = 1
            LOG.info("::::Has initialed 10 flows")
