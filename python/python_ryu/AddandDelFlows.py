...
class AddFlow(app_manager.RyuApp):
    ...
    def installDefaultFlow(self,datapath):
        ...
        match = self.match_v12(datapath, in_port=self.inport,dl_dst="ff:ff:ff:ff:ff:ff",dl_src="22:11:11:11:11:11",dl_type=0x0806,arp_spa="10.10.10.100",arp_tpa="10.10.10.200")
        actions = self.action_v12(datapath, out_port=self.output3)
        self.flow_v12(datapath, priority=3000,match=match,actions=actions)

    def installFlow(self,datapath):
        match = self.match_v12(datapath,in_port=self.inport,dl_dst="ff:ff:ff:ff:ff:ff",dl_src="22:11:11:11:11:11")
        actions = self.action_v12(datapath, out_port=self.output4)
        self.flow_v12(datapath, priority=2000,match=match,actions=actions)
                 
    def removeFlow(self,datapath):
        #remove controller flow
        match_del = self.match_v12(datapath,in_port=self.inport,dl_dst="22:00:00:00:00:00")
    ...
    def match_v12(self, datapath, in_port=None, dl_dst=None, dl_src=None, dl_type=None, 
                 ...
                 udp_src=None, udp_dst=None, arp_tpa=None, arp_spa=None):
        ...
        if arp_spa is not None:
            arp_spa = self.ipv4_to_int(arp_spa)
    ...
    def switch_features_handler(self, ev):
        ...
        time.sleep(2)
        LOG.info("::::Instal a forward flow")
        self.installDefaultFlow(datapath)
        #time.sleep(60)
        LOG.info("::::Instal a new forward flow")
