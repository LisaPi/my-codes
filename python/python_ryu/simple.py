...
global pt1_avr, pt2_avr, pt3_avr
pt1_avr = pt2_avr = pt3_avr = 0
...
class SimpleSwitch14(app_manager.RyuApp):
    ...
    rsv_bandw = 5000000
    size = 64
    maxpck = (bandwidth-rsv_bandw)/8/(size+8+12)
    offset_pkt = 100
    ...
    def port_stats_count(self, datapath):
        ...
        if float(pt1_sum + pt2_sum)/2/self.maxpck > 1.05:
            print("port 3 has congested")
            print(abs(pt1_sum/2-pt1_avr))
            print(abs(pt2_sum/2-pt2_avr))
            if (abs(pt1_sum/2-pt1_avr) > self.offset_pkt) | (abs(pt2_sum/2-pt2_avr) > self.offset_pkt):
                print("aaaaaaaaaaaaa")
                ...
                actions = [parser.OFPActionOutput(self.pt3,
                                          ofproto.OFPCML_NO_BUFFER)]  
                self.add_meter_flow(datapath, ofproto.OFPFC_MODIFY, 1000, match, ofproto.OFPP_ANY, 
                             ofproto.OFPG_ANY,actions)
        else:
            if abs(pt1_sum/2-pt1_avr) > self.offset_pkt | abs(pt2_sum/2-pt2_avr) > self.offset_pkt:
                match = parser.OFPMatch(in_port=self.pt2, eth_src='00:00:00:22:22:22', eth_type=0x0800, 
                                   ipv4_src='192.168.2.100')
